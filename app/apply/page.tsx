...
export default function AdmissionPage() {
  ...
  async function onSubmit(data: AdmissionFormValues) {
    ...
    try {
      const response = await fetch("/api/admission", {
        ...
        body: JSON.stringify({
          ...data,
          dob: format(data.dob, "yyyy-MM-dd"), // Laravel requirement
        }),
      });
      ...
      if (response.ok) {
        ...
        localStorage.setItem("latest_admission", JSON.stringify(result.admission));
        //router.push("/apply/success");
        router.push(`/apply/success?id=${result.admission.id}`);
      } else {
  ...
  // DEBUGGING: If clicking submit does nothing, check your browser console!
      ...
      <main className="flex-1 py-10 px-4">
        <div className="container mx-auto max-w-4xl">
          <div className="mb-8 text-center">
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Admission Form 2026</h1>
            <p className="text-slate-500 mt-2">Enter the required details to apply for admission.</p>
          </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit, onInvalid)} className="space-y-8">
              ...
              <Button type="submit" className="w-full h-14 text-lg font-bold shadow-md hover:shadow-lg transition-all" disabled={loading}>
                {loading ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Submitting Application...</> : "Apply Now"}
              </Button>
            </form>
          </Form>
        </div>
